...
const CLEVERTAP_TEMPLATE = {
  ...
  description:
    'Production-ready CleverTap integration with profile uploads, event tracking, and MSG-* communication preferences. Supports 8 validated event types with rich field mappings.',
  category: 'CRM',
...
if (require.main === module) {
  ...
  const orgUnitRid = args[0] ? parseInt(args[0], 10) : null;
    ...
    seedCleverTapTemplateForAll()
      .then((results) => {
        ...
        results.forEach((result) => {
          if (result.success) {
            console.log(
              `   ✓ Org Unit ${result.orgUnitRid}: ${result.isNew ? 'Created' : 'Updated'} (${result.templateId})`
            );
          } else {
