...
router.post('/cleanup/stuck-retrying', asyncHandler(async (req, res) => {
...
router.post('/:id/replay', asyncHandler(async (req, res) => {
  try {
...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const [logs, total] = await Promise.all([
    data.listLogs(req.orgId, filters),
    data.countLogs(req.orgId, filters)
  ]);
...
router.get('/export', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await data.streamLogsForExport(req.orgId, filters, async (logEntry) => {
      if (isAborted()) {
        return;
      }
      ...
      // Build CSV row
      const flowLabel = logEntry.direction === 'OUTBOUND' && logEntry.triggerType === 'SCHEDULED'
        ? 'SCHEDULED'
      ...
      const canContinue = await writeWithBackpressure(
        req,
        res,
...
router.post('/export/selected', asyncHandler(async (req, res) => {
  try {
    ...
    for (const logEntry of logs) {
      if (logEntry.__KEEP___KEEP_integrationConfig__Id__ && !integrationCache.has(logEntry.__KEEP___KEEP_integrationConfig__Id__)) {
        try {
          const integration = await data.getIntegrationById(logEntry.__KEEP___KEEP_integrationConfig__Id__);
          if (integration) {
            integrationCache.set(logEntry.__KEEP___KEEP_integrationConfig__Id__, integration);
    ...
    if (format === 'csv') {
      ...
      const cleanLogs = logs.map(({ tenantId, ...rest }) => {
        const flow = rest.direction === 'OUTBOUND' && rest.triggerType === 'SCHEDULED'
          ? 'SCHEDULED'
...
router.get('/:id', asyncHandler(async (req, res) => {
  ...
  if (logEntry.__KEEP___KEEP_integrationConfig__Id__) {
    try {
      ...
      if (__KEEP_integrationConfig__ && __KEEP_integrationConfig__.outgoingAuthConfig) {
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (!payload.__KEEP___KEEP_integrationConfig__Id__) {
    return res.status(400).json({ error: '__KEEP___KEEP_integrationConfig__Id__ is required', code: 'VALIDATION_ERROR' });
  }
  await data.recordLog(req.orgId, payload);
  log('info', 'Log recorded', { __KEEP___KEEP_integrationConfig__Id__: payload.__KEEP___KEEP_integrationConfig__Id__ });
