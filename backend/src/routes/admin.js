...
router.post('/rate-limits/bulk-apply', asyncHandler(async (req, res) => {
  ...
  const affectedIds = await db.collection('integration_configs')
...
router.post('/rate-limits/bulk-reset', asyncHandler(async (req, res) => {
  ...
  await auditConfig.rateLimitBulkReset(req, ids.map(id => id.toString()));
...
router.patch('/system-config', asyncHandler(async (req, res) => {
  ...
  if (input.security && typeof input.security === 'object') {
    patch.security = {
      ...
      blockPrivateNetworks: input.security.blockPrivateNetworks ?? stored.security?.blockPrivateNetworks ?? config.security.blockPrivateNetworks
    };
  }
