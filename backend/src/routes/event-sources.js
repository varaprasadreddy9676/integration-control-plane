...
// ---------------------------------------------------------------------------
router.post('/test', requirePermission('event_source:manage'), asyncHandler(async (req, res) => {
  ...
  const effectiveConfig = type === 'mysql'
    ? sanitizeMysqlSourceConfig(sourceConfig)
    : sourceConfig;
...
// ---------------------------------------------------------------------------
router.get('/:orgId/columns', requirePermission('event_source:view'), asyncHandler(async (req, res) => {
...
router.get('/', requirePermission('event_source:view'), asyncHandler(async (req, res) => {
  const { role, orgId: userOrgId } = req.user;
...
router.get('/:orgId', requirePermission('event_source:view'), asyncHandler(async (req, res) => {
  const orgId = parseInt(req.params.orgId, 10);
  if (!Number.isFinite(orgId)) {
...
router.put('/:orgId', requirePermission('event_source:manage'), asyncHandler(async (req, res) => {
  const orgId = parseInt(req.params.orgId, 10);
  if (!Number.isFinite(orgId)) {
    return res.status(400).json({ error: 'Invalid orgId', code: 'VALIDATION_ERROR' });
  }
  ...
  manager._syncAdapters().catch(err =>
    log('warn', 'Adapter sync after config update failed', { error: err.message })
  );
...
router.delete('/:orgId', requirePermission('event_source:manage'), asyncHandler(async (req, res) => {
  const orgId = parseInt(req.params.orgId, 10);
  if (!Number.isFinite(orgId)) {
    return res.status(400).json({ error: 'Invalid orgId', code: 'VALIDATION_ERROR' });
  }
  ...
  const manager = getDeliveryWorkerManager();
  manager._syncAdapters().catch(err =>
    log('warn', 'Adapter sync after config delete failed', { error: err.message })
  );
