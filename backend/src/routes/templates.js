...
router.post(
  ...
  asyncHandler(async (req, res) => {
    try {
      const { templateId } = req.params;
      const { newName, newDescription, modifications = {} } = req.body;
      ...
      const __KEEP_integrationConfig__ = await templates.createIntegrationFromTemplate(req.orgId, templateId, {
        name: newName,
