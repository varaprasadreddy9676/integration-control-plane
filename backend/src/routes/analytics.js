...
async function aggregateOverviewCollection(db, collection, match, fields) {
  const statusAgg = await db.collection(collection).aggregate([
    { $match: match },
    { $group: { _id: `$${fields.statusField}`, count: { $sum: 1 } } }
  ]).toArray();

  const totalAgg = await db.collection(collection).aggregate([
    { $match: match },
    { $count: 'total' }
  ]).toArray();

  const eventTypeAgg = await db.collection(collection).aggregate([
    { $match: match },
    { $group: { _id: `$${fields.eventTypeField}`, count: { $sum: 1 } } },
    { $sort: { count: -1 } }
  ]).toArray();

  const integrationAgg = await db.collection(collection).aggregate([
    { $match: match },
    {
      $group: {
        ...
        failed: {
          $sum: {
            $cond: [{ $in: [`$${fields.statusField}`, ['FAILED', 'failed', 'SKIPPED', 'skipped', 'ABANDONED', 'abandoned']] }, 1, 0]
          }
        },
        responseTimeSum: {
          $sum: {
  ...
  const responseAgg = await db.collection(collection).aggregate([
    { $match: match },
  ...
  const bucketAgg = await db.collection(collection).aggregate([
    { $match: match },
    { $match: { [fields.responseTimeField]: { $gt: 0 } } },
    {
      $bucket: {
...
async function aggregatePerformanceCollection(db, collection, match, fields) {
  ...
  const successAgg = await db.collection(collection).aggregate([
    { $match: match },
...
function bucketAggToIndexCounts(bucketAgg) {
  return bucketAgg.map(bucket => {
    const idx = RESPONSE_BUCKETS.findIndex(boundary => boundary === bucket._id);
...
router.get('/overview', asyncHandler(async (req, res) => {
  try {
    ...
    for (const item of execution.integrationAgg) {
      ...
      integrationMap[id].__KEEP_integrationName__ = integrationMap[id].__KEEP_integrationName__ || item.integrationName || null;
    ...
    const responseTimeSum = (execution.responseAgg.avgResponseTime || 0) * (execution.responseAgg.responseTimeCount || 0);
...
router.get('/timeseries', asyncHandler(async (req, res) => {
  try {
    ...
    const executionSeries = await aggregateTimeseriesCollection(db, 'execution_logs', executionMatch, executionFields, interval);
...
router.get('/performance', asyncHandler(async (req, res) => {
  try {
    ...
    const responseTimeSum = (executionPerf.responseAgg.avgResponseTime || 0) * (executionPerf.responseAgg.responseTimeCount || 0);
