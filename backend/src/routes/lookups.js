...
router.get('/', asyncHandler(async (req, res) => {
  ...
  if (Object.prototype.hasOwnProperty.call(req.query, 'tenantId')) {
    return res.status(400).json({
      success: false,
  ...
  const filters = {
    type: req.query.type,
    orgUnitRid: req.query.orgUnitRid !== undefined
      ? parseInt(req.query.orgUnitRid, 10)
      : undefined,
...
router.post('/import', upload.single('file'), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await auditLookup.bulkImported(req, type, importResult.inserted || importResult.count || importResult.created || parseResult.lookups.length);
