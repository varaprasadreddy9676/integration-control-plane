  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      if (validateFirst) {
        ...
        const validIndexes = validationResults.filter((r) => r.valid).map((r) => r.index);

        const validIntegrations = validIndexes.map((i) => integrations[i]);
        const invalidIndexes = validationResults.filter((r) => !r.valid).map((r) => r.index);
    ...
    try {
      ...
      if (validateFirst) {
        ...
        for (let i = 0; i < validationResults.length; i++) {
          ...
          try {
            const updatedIntegration = await data.updateIntegration(req.orgId, update.id, update);
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      for (let i = 0; i < integrationIds.length; i++) {
        ...
        try {
          const updatedIntegration = await data.updateIntegration(req.orgId, integrationId, { isActive });
...
function validateIntegration(integration) {
  ...
  if (
    ...
    (typeof integration.timeoutMs !== 'number' || integration.timeoutMs < 1000 || integration.timeoutMs > 60000)
  ) {
  ...
  if (
    ...
    (typeof integration.retryCount !== 'number' || integration.retryCount < 0 || integration.retryCount > 10)
  ) {
  ...
  if (
    authType &&
    !['NONE', 'API_KEY', 'BEARER', 'BEARER_TOKEN', 'OAUTH2', 'BASIC', 'CUSTOM', 'CUSTOM_HEADERS'].includes(authType)
  ) {
...
function validateIntegrationUpdate(update) {
  ...
  if (
    update.timeoutMs &&
    (typeof update.timeoutMs !== 'number' || update.timeoutMs < 1000 || update.timeoutMs > 60000)
  ) {
    warnings.push('Timeout should be a number between 1000 and 60000 milliseconds');
  }
  ...
  if (
    updateAuthType &&
    !['NONE', 'API_KEY', 'BEARER', 'BEARER_TOKEN', 'OAUTH2', 'BASIC', 'CUSTOM', 'CUSTOM_HEADERS'].includes(
      updateAuthType
    )
  ) {
    errors.push('Auth type must be one of: NONE, API_KEY, BEARER, OAUTH2, BASIC, CUSTOM, CUSTOM_HEADERS');
