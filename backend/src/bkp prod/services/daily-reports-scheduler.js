...
class DailyReportsScheduler {
  ...
  /**
   * Run daily reports for all configured entities
   */
  async runDailyReports() {
    ...
    try {
      ...
      // Fetch all entities that have daily reports enabled
      ...
      if (reportConfigs.length === 0) {
        log('info', '[DailyReports] No entities configured for daily reports');
        return;
      }
      ...
      for (const reportConfig of reportConfigs) {
        try {
          ...
          await logError(error, { scope: 'DailyReports', action: 'processEntity', entityParentRid: reportConfig.entityParentRid });
          results.push({
            entityParentRid: reportConfig.entityParentRid,
            success: false,
            error: error.message
          });
        }
  ...
  /**
   * Send daily report for a single entity
   * @param {Object} reportConfig - Report configuration
   * @returns {Promise<Object>} Result
   */
  async sendDailyReport(reportConfig) {
    const { entityParentRid, entityName, recipients } = reportConfig;

    log('info', `[DailyReports] Generating report for entity ${entityParentRid} (${entityName})`);
    ...
    try {
      ...
      const dashboardUrl = `${frontendUrl}/dashboard?entityParentRid=${entityParentRid}&days=1`;

      // Optionally generate PDF attachment
      let pdfBuffer = null;
      if (dailyReportsConfig.includePdf) {
        try {
          log('info', `[DailyReports] Generating PDF for entity ${entityParentRid}`);
          ...
          await logError(pdfError, { scope: 'DailyReports', action: 'generatePDF', entityParentRid });
      ...
      log('info', `[DailyReports] Report sent successfully for entity ${entityParentRid}`);
      ...
      await logError(error, { scope: 'DailyReports', action: 'sendDailyReport', entityParentRid });
      return {
        entityParentRid,
        entityName,
        success: false,
