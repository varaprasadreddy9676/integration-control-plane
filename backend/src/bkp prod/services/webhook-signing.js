 ...
 * Implements Standard Webhooks specification for payload signing
...
// Constants following Standard Webhooks specification
...
/**
 * Sign a webhook payload following Standard Webhooks specification
 *
 * Signature scheme:
 ...
 * @param {string} payload - The webhook payload as string (JSON.stringify result)
...
function signPayload(secret, messageId, timestamp, payload) {
  try {
    ...
    // This follows Standard Webhooks spec exactly
    ...
    // Format: v1,signature (Standard Webhooks format)
    return `${SIGNATURE_VERSION},${signature}`;
  } catch (error) {
    log('error', 'Failed to sign webhook payload', {
      error: error.message,
      messageId,
      timestamp
    });
 ...
 * @param {number} timestamp - Unix timestamp in seconds
 * @param {string} payload - The webhook payload as string
...
function generateSignatureHeaders(secrets, messageId, timestamp, payload) {
  try {
    ...
    // Return Standard Webhooks headers
    return {
...
const isValid = verifyWebhook(secret, req.headers, req.body);`,
...
is_valid = verify_webhook(secret, request.headers, request.data)`,

    php: `<?php
// PHP signature verification
function verifyWebhook($secret, $headers, $rawBody) {
    $messageId = $headers['x-webhook-id'];
...
// Example usage:
$secret = '${exampleSecret}';
$isValid = verifyWebhook($secret, getallheaders(), file_get_contents('php://input'));
?>`,
...
func VerifyWebhook(secret string, headers map[string]string, rawBody string) error {
    messageID := headers["X-Webhook-ID"]
    timestamp := headers["X-Webhook-Timestamp"]
    signature := headers["X-Webhook-Signature"]
...
// Example usage:
secret := "${exampleSecret}"
err := VerifyWebhook(secret, headers, requestBody)`
  };
}
