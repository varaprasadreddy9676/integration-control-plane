  ...
  try {
    ...
    rows.forEach((row, i) => {
      ...
      try {
        ...
        const tenantId = parseInt(row.tenantId || row.tenant_id || row.TenantId, 10);
        const payloadRaw = row.payload || row.Payload;

        // Validate required fields
        if (!eventType || !tenantId || !payloadRaw) {
          errors.push({
            row: rowNum,
            error: 'Missing required fields: eventType, tenantId, payload'
          });
          return;
        }
