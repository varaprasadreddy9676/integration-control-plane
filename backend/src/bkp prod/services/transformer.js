    ...
    switch (row.transform) {
      ...
      case 'lookup':
        if (row.lookupType && sourceVal) {
          ...
          val = await resolveLookup(sourceVal, row.lookupType, event.orgId, event.tenantId);
...
async function applyTransform(integration, payload, context = {}) {
  ...
  const event = {
    orgId: context.orgId || integration.tenantId,
...
async function applyScriptTransform(script, payload, context) {
  try {
    ...
    // Create a secure transformation function
    // Return the function expression so vm.run() returns the function
    const secureScript = `
    ...
    // Execute in secure sandbox (vm.run returns a Promise)
    const transform = await vm.run(secureScript);

    if (typeof transform === 'function') {
      const result = await transform(payload, { ...context, http });
      if (depth(result) > MAX_DEPTH) {
        throw new Error('Transformed object too deep');
      }
      return result;
    } else {
      throw new Error('Transform function not found in script');
    }
 ...
 * @param {Object} context - Context object with tenantId, etc.
