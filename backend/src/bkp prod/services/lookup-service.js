  ...
  if (Array.isArray(array)) {
    for (const item of array) {
      ...
      if (sourceValue !== null && sourceValue !== undefined && sourceValue !== '') {
        const mappedValue = await resolveLookup(sourceValue, type, entityParentRid, entityRid);
 ...
 */
async function applyLookupConfig(payload, config, entityParentRid, entityRid) {
...
async function applyLookups(payload, lookupConfigs, event) {
  ...
  const entityRid = event.entityRid || event.entity_rid;
  ...
  for (const config of lookupConfigs) {
    try {
      payload = await applyLookupConfig(payload, config, entityParentRid, entityRid);
    } catch (err) {
 ...
 */
async function testLookups(payload, lookupConfigs, entityParentRid, entityRid) {
  const errors = [];
  let testPayload = JSON.parse(JSON.stringify(payload)); // Deep clone

  const event = { entityParentRid, entityRid };

  for (let i = 0; i < lookupConfigs.length; i++) {
    const config = lookupConfigs[i];
    try {
      testPayload = await applyLookupConfig(testPayload, config, entityParentRid, entityRid);
    } catch (err) {
      errors.push({
        index: i,
        type: config.type,
