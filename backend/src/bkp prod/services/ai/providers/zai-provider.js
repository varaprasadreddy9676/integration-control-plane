/**
 * Z.ai Provider (GLM-4.7) - OpenAI-compatible API
 */
...
class ZAIProvider extends BaseAIProvider {
  ...
  async _complete(messages, { temperature = 0.3, maxTokens, jsonMode = false } = {}) {
    if (!this.client) throw new Error('Z.ai provider not configured or OpenAI SDK not installed');
    const params = {
      model: this.model,
      messages,
      temperature,
  ...
  async testConnection() {
    ...
    try {
      ...
      throw new Error(`Z.ai connection failed: ${error.message}`);
    }
  }
  ...
  async generateTestPayload(eventType, entityParentRid) {
    const prompt = await buildTestPayloadPrompt(eventType, entityParentRid);
    try {
      const text = await this._complete([
        { role: 'system', content: getSystemPrompt() },
        { role: 'user', content: prompt }
