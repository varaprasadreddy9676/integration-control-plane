 ...
 * @param {number} data.entityParentRid - Entity ID
...
async function logInteraction(data) {
  try {
    ...
    const interaction = {
      entityParentRid: data.entityParentRid,
...
/**
 * Get AI interactions for an entity
 * @param {number} entityParentRid - Entity ID
 ...
 */
async function getInteractions(entityParentRid, options = {}) {
  if (!mongodb.isConnected()) {
    return [];
  }
 ...
 */
async function getStats(entityParentRid, days = 7) {
  if (!mongodb.isConnected()) {
    return null;
  }
...
async function createIndexes() {
  ...
  await collection.createIndex({ entityParentRid: 1, createdAt: -1 });
