...
class VersionManager {
  ...
  async getIntegrationVersions(__KEEP_integrationName__, entityRid, limit = 50, includeInactive = false, includePrerelease = false) {
    const cacheKey = `versions_${entityRid}_${__KEEP_integrationName__}_${limit}_${includeInactive}_${includePrerelease}`;
    ...
    try {
      const integrations = await this.data.listIntegrations(entityRid);
  ...
  async getVersionCompatibilityMatrix(__KEEP_integrationName__, entityRid) {
    try {
      const integrations = await this.data.listIntegrations(entityRid);
  ...
  async getUpgradePath(__KEEP_integrationName__, fromVersion, toVersion, entityRid) {
    try {
      const integrations = await this.data.listIntegrations(entityRid);
