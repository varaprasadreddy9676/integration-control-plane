...
function validateLookupEntry(lookup) {
  ...
  // entityRid must be a number or null
  if (lookup.entityRid !== undefined && lookup.entityRid !== null && typeof lookup.entityRid !== 'number') {
    errors.push('entityRid must be a number or null');
  }
...
function validateBulkImport(lookups) {
  ...
  for (let i = 0; i < lookups.length; i++) {
    ...
    const key = `${lookup.type}:${lookup.entityRid || 'null'}:${lookup.source.id}`;
