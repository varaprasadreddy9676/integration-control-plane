...
class DashboardCaptureService {
   ...
   * @param {number} options.entityParentRid - Entity RID
   * @param {string} options.apiKey - API key for authentication
   * @param {number} options.days - Time range (1 for today)
   * @returns {Promise<Buffer>} PDF buffer
   */
  async captureDashboard({ entityParentRid, apiKey, days = 1 }) {
    ...
    try {
      ...
      const dashboardUrl = `${frontendUrl}/dashboard?entityParentRid=${entityParentRid}&days=${days}`;
      ...
      log('info', `[DashboardCapture] Dashboard captured successfully for entity ${entityParentRid}`);

      return pdfBuffer;
    } catch (error) {
      await logError(error, { scope: 'DashboardCapture', action: 'captureDashboard', entityParentRid });
      throw error;
    } finally {
      if (page) {
        await page.close();
   ...
   */
  async captureDashboardPNG({ entityParentRid, apiKey, days = 1 }) {
    const browser = await this.initializeBrowser();
    let page = null;
    ...
    try {
      ...
      log('info', `[DashboardCapture] Dashboard PNG captured successfully for entity ${entityParentRid}`);

      return pngBuffer;
    } catch (error) {
      await logError(error, { scope: 'DashboardCapture', action: 'captureDashboardPNG', entityParentRid });
      throw error;
    } finally {
      if (page) {
        await page.close();
   ...
   */
  async getDashboardSummary({ entityParentRid, apiKey, days = 1 }) {
    try {
      // Use internal data layer directly instead of making HTTP request
      const data = require('../data');
      const dashboardData = await data.getDashboardSummary(entityParentRid);

      log('info', `[DashboardCapture] Summary fetched for entity ${entityParentRid}`);
      ...
      await logError(error, { scope: 'DashboardCapture', action: 'getDashboardSummary', entityParentRid });
      return {
        totalDeliveries: 0,
