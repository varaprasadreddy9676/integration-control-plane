...
class HealthMonitor {
  ...
  async getSystemHealth(entityRid) {
    try {
      const [
        summary,
        integrationMetrics,
        ...
        data.getDashboardSummary(entityRid),
        analyticsAggregator.getIntegrationMetrics(entityRid, { days: 1 }),
        analyticsAggregator.getErrorTrends(entityRid, 1),
        this.getMemoryStats(),
        this.getUptimeStats()
      ]);
  ...
  async getIntegrationHealthHistory(entityRid, days = 7) {
    try {
      const metrics = await analyticsAggregator.getIntegrationMetrics(entityRid, {
        days,
        includeHourly: true
      });
