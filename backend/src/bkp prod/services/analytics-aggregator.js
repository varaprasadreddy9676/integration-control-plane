...
class AnalyticsAggregator {
  ...
  getCacheKey(tenantId, type, params) {
    return `${tenantId}:${type}:${JSON.stringify(params)}`;
  }
  ...
  async getIntegrationMetrics(tenantId, options = {}) {
    const {
      days = 30,
      integrationId,
      eventTypes = [],
    ...
    const cacheKey = this.getCacheKey(tenantId, 'integrationMetrics', options);
    ...
    try {
      ...
      const logs = await data.listLogs(tenantId, filters);
  ...
  async getErrorTrends(tenantId, days = 7) {
    const cacheKey = this.getCacheKey(tenantId, 'errorTrends', { days });
    const cached = this.getFromCache(cacheKey);
    if (cached) return cached;
