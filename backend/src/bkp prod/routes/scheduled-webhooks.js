...
/**
 * GET /api/v1/scheduled-webhooks
 * List scheduled webhooks with optional filters
 */
router.get('/', asyncHandler(async (req, res) => {
  const filters = {
    status: req.query.status, // PENDING | SENT | FAILED | CANCELLED | OVERDUE
    webhookConfigId: req.query.webhookConfigId,
    eventType: req.query.eventType,
    limit: parseInt(req.query.limit) || 500
  };

  const scheduledWebhooks = await data.listScheduledWebhooks(req.entityParentRid, filters);

  log('info', 'Listed scheduled webhooks', {
    entityRid: req.entityParentRid,
    count: scheduledWebhooks.length,
    filters
  });

  res.json({ scheduledWebhooks });
}));

/**
 * POST /api/v1/scheduled-webhooks/validate
...
/**
 * DELETE /api/v1/scheduled-webhooks/bulk
 * Bulk delete/cancel scheduled webhooks
...
router.delete('/bulk', asyncHandler(async (req, res) => {
  ...
  for (const id of ids) {
    try {
      const success = await data.deleteScheduledWebhook(req.entityParentRid, id);
      if (success) {
        deletedCount++;
      } else {
        failedIds.push(id);
  ...
  log('info', 'Bulk scheduled webhooks deletion', {
    entityRid: req.entityParentRid,
    total: ids.length,
  ...
  res.json({
    message: `Successfully deleted ${deletedCount} scheduled webhook(s)`,
    deletedCount,
    failedIds
  });
}));
...
/**
 * DELETE /api/v1/scheduled-webhooks/:id
...
router.delete('/:id', asyncHandler(async (req, res) => {
  ...
  if (!success) {
    return res.status(404).json({
      error: 'Scheduled webhook not found',
      code: 'NOT_FOUND'
    });
  }

  log('info', 'Scheduled webhook cancelled', {
    entityRid: req.entityParentRid,
    scheduledWebhookId: id
  });

  res.json({
    message: 'Scheduled webhook cancelled successfully'
  });
}));
