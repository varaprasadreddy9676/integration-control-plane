...
const normalizeRateLimits = (input = {}, fallback = {}) => {
  ...
  const maxRequests = Number.isFinite(Number(maxRequestsRaw))
    ...
    : 100;
  const windowSeconds = Number.isFinite(Number(windowSecondsRaw))
    ? Math.max(1, Number(windowSecondsRaw))
    : 60;
...
const buildRateLimitQuery = (filters = {}) => {
  ...
  if (orgId) {
    conditions.push({ tenantId: orgId });
  }
...
router.patch('/users/:id/disable', asyncHandler(async (req, res) => {
  const { id } = req.params;
...
router.get('/orgs', asyncHandler(async (_req, res) => {
  const orgs = await data.listTenantIds();
  res.json({ orgs });
}));

// GET /api/v1/admin/orgs/summary
router.get('/orgs/summary', asyncHandler(async (_req, res) => {
...
router.post('/orgs', asyncHandler(async (req, res) => {
  ...
  if (parsedOrgId) {
    ...
    const unitCollision = await mongodb.getDbSafe().then(dbClient => dbClient.collection('org_units').findOne({ rid: parsedOrgId }));
    if (unitCollision) {
...
router.put('/orgs/:orgId', asyncHandler(async (req, res) => {
  const orgId = parsePositiveInt(req.params.orgId);
  if (!orgId) {
    throw new ValidationError('orgId must be a positive integer');
  }
...
router.delete('/orgs/:orgId', asyncHandler(async (req, res) => {
  const orgId = parsePositiveInt(req.params.orgId);
  if (!orgId) {
    throw new ValidationError('orgId must be a positive integer');
  }
...
router.get('/orgs/:orgId/units', asyncHandler(async (req, res) => {
  const orgId = parsePositiveInt(req.params.orgId);
  if (!orgId) {
    throw new ValidationError('orgId must be a positive integer');
  }
...
router.post('/orgs/:orgId/units', asyncHandler(async (req, res) => {
  const orgId = parsePositiveInt(req.params.orgId);
  if (!orgId) {
    throw new ValidationError('orgId must be a positive integer');
  }
  ...
  if (parsedRid) {
    ...
    const existingUnit = await mongodb.getDbSafe().then(dbClient => dbClient.collection('org_units').findOne({ rid: parsedRid }));
    if (existingUnit) {
      throw new ValidationError('rid already exists');
    }

    const orgIdCollision = await mongodb.getDbSafe().then(dbClient => dbClient.collection('organizations').findOne({ orgId: parsedRid }));
    if (orgIdCollision) {
...
router.put('/orgs/:orgId/units/:rid', asyncHandler(async (req, res) => {
  const orgId = parsePositiveInt(req.params.orgId);
...
router.delete('/orgs/:orgId/units/:rid', asyncHandler(async (req, res) => {
  const orgId = parsePositiveInt(req.params.orgId);
  const rid = parsePositiveInt(req.params.rid);
  if (!orgId || !rid) {
    throw new ValidationError('orgId and rid must be positive integers');
...
router.get('/rate-limits', asyncHandler(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    db.collection('integration_configs')
      .find(query)
      .sort({ updatedAt: -1 })
      .skip(offset)
      .limit(limit)
  ...
  const results = await Promise.all(items.map(async (doc) => {
    const rateLimits = doc.rateLimits || null;
    let status = null;
    if (rateLimits && rateLimits.enabled) {
      status = normalizeRateLimitStatus(await getRateLimitStatus(doc._id.toString(), doc.tenantId));
    } else {
...
router.post('/rate-limits/bulk-apply', asyncHandler(async (req, res) => {
...
router.post('/rate-limits/bulk-reset', asyncHandler(async (req, res) => {
...
router.get('/rate-limits/export', asyncHandler(async (req, res) => {
  const orgId = req.query.orgId ? Number(req.query.orgId) : undefined;
  const direction = req.query.direction ? String(req.query.direction).toUpperCase() : undefined;
  const enabledFilter = req.query.enabled;
  const search = req.query.search ? String(req.query.search).trim() : undefined;
  ...
  const query = buildRateLimitQuery({ orgId, direction, enabled: enabledFilter, search });
  const db = await mongodb.getDbSafe();
  const items = await db.collection('integration_configs')
    .find(query)
    .sort({ updatedAt: -1 })
    .toArray();
...
router.get('/audit-logs', asyncHandler(async (req, res) => {
  ...
  const trendQuery = conditions.length > 0
...
router.get('/audit-logs/export', asyncHandler(async (req, res) => {
  ...
  const db = await mongodb.getDbSafe();
  const items = await db.collection('admin_audit')
    .find(query)
    .sort({ createdAt: -1 })
    .limit(5000)
    .toArray();
...
router.get('/audit-logs/export-trend', asyncHandler(async (req, res) => {
  const action = req.query.action ? String(req.query.action).trim() : undefined;
  const role = req.query.role ? String(req.query.role).trim() : undefined;
  const adminId = req.query.adminId ? String(req.query.adminId).trim() : undefined;
  const search = req.query.search ? String(req.query.search).trim() : undefined;
  ...
  const db = await mongodb.getDbSafe();
  const rows = await db.collection('admin_audit')
    .aggregate([
      { $match: trendQuery },
      {
        $group: {
  ...
  const csv = [
    headers.join(','),
    ...dailyCounts.map((row) => `${escapeCsv(row.date)},${row.count}`)
  ].join('\n');
...
router.patch('/rate-limits/:id', asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  await db.collection('integration_configs').updateOne(
    { _id: mongodb.toObjectId(id) },
    { $set: { rateLimits: nextLimits, updatedAt: new Date() } }
  );

  log('info', 'Admin updated rate limits', {
    adminId: req.user?.id,
    integrationId: id,
    orgId: existing.tenantId,
...
router.post('/rate-limits/:id/reset', asyncHandler(async (req, res) => {
  const { id } = req.params;
  const db = await mongodb.getDbSafe();
  ...
  const result = await resetRateLimit(id, integration.tenantId);
  res.json({ success: true, ...result });
}));

// GET /api/v1/admin/ui-config
router.get('/ui-config', asyncHandler(async (_req, res) => {
  const config = await data.getUiConfigDefault();
  res.json({ config });
}));

// PATCH /api/v1/admin/ui-config
router.patch('/ui-config', asyncHandler(async (req, res) => {
...
router.get('/system-config', asyncHandler(async (_req, res) => {
  const safeConfig = {
    security: {
...
router.patch('/system-config', asyncHandler(async (req, res) => {
  const input = req.body?.config || req.body || {};
  if (!input || typeof input !== 'object') {
    throw new ValidationError('config object required');
  }
  ...
  if (input.security && typeof input.security === 'object') {
    nextConfig.security = {
      ...(fileConfig.security || {}),
      enforceHttps: input.security.enforceHttps ?? (fileConfig.security || {}).enforceHttps,
      blockPrivateNetworks: input.security.blockPrivateNetworks ?? (fileConfig.security || {}).blockPrivateNetworks
    };
  }

  if (input.worker && typeof input.worker === 'object') {
    nextConfig.worker = { ...(fileConfig.worker || {}), ...input.worker };
  }

  if (input.scheduler && typeof input.scheduler === 'object') {
    nextConfig.scheduler = { ...(fileConfig.scheduler || {}), ...input.scheduler };
  }

  if (input.eventSource && typeof input.eventSource === 'object') {
    nextConfig.eventSource = { ...(fileConfig.eventSource || {}), ...input.eventSource };
  }

  if (input.kafka && typeof input.kafka === 'object') {
    nextConfig.kafka = { ...(fileConfig.kafka || {}), ...input.kafka };
  }
  ...
  res.json({ message: 'Config updated. Restart backend to apply changes.', requiresRestart: true });
}));
 ...
 */
router.post('/force-gc', asyncHandler(async (req, res) => {
  const memoryMonitor = new MemoryMonitor();
