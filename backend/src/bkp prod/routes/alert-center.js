...
router.get('/', asyncHandler(async (req, res) => {
  const entityParentRid = req.entityParentRid || Number(req.query.entityParentRid);
  if (!entityParentRid) {
    return res.status(400).json({ error: 'entityParentRid missing', code: 'VALIDATION_ERROR' });
  }
  ...
  const logs = await data.listAlertCenterLogs(entityParentRid, filters);
  return res.json({ logs });
}));

router.get('/status', asyncHandler(async (req, res) => {
  const entityParentRid = req.entityParentRid || Number(req.query.entityParentRid);
  const status = await data.getFailureReportSchedulerStatus(entityParentRid);
