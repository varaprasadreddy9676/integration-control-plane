 ...
 *
 * Manage per-entity AI provider settings and API keys.
 ...
 * Get AI configuration for the entity (masked - no raw API key)
 */
router.get(
  '/',
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!entityParentRid) {
      throw new ValidationError('entityParentRid is required');
    }

    let entityConfig = null;
    try {
      entityConfig = await aiConfigData.getAIConfig(entityParentRid);
    } catch (err) {
 ...
 * Save AI configuration for the entity
 */
router.put(
  '/',
  ...
  asyncHandler(async (req, res) => {
    ...
    const saved = await aiConfigData.saveAIConfig(entityParentRid, {
      provider,
      apiKey,
      model,
    ...
    if (aiService.invalidateProviderCache) {
      aiService.invalidateProviderCache(entityParentRid);
    }
 ...
 * Test the AI connection for this entity
 */
router.post(
  '/test',
  requireFeature(FEATURES.AI_CONFIG, 'configure'),
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    await aiConfigData.deleteAIKey(entityParentRid);
