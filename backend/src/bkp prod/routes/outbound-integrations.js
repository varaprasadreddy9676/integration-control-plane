...
router.get('/:id', asyncHandler(async (req, res) => {
  ...
  if (!integration || integration.tenantId !== req.orgId) {
...
router.get('/:id/curl', asyncHandler(async (req, res) => {
  const integration = await data.getIntegration(req.params.id);
  if (!integration || integration.tenantId !== req.orgId) {
    return res.status(404).json({ error: 'Integration not found', code: 'NOT_FOUND' });
  }
...
router.post('/:id/duplicate', asyncHandler(async (req, res) => {
  ...
  const { id, tenantId, entityName, createdAt, updatedAt, ...duplicateData } = integration;
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (req.body.scope === 'INCLUDE_CHILDREN') {
      try {
        ...
        const invalidRids = req.body.excludedEntityRids.filter(rid =>
          !childRids.includes(rid) && rid !== req.orgId
        );
  ...
  log('info', 'Integration created', { id: integration.id, tenantId: req.orgId, isMultiAction });
...
router.put('/:id', asyncHandler(async (req, res) => {
  ...
  log('info', 'Integration updated', { id: integration.id, tenantId: req.orgId });
  return res.json(integration);
}));
...
router.delete('/:id', asyncHandler(async (req, res) => {
  ...
  log('info', 'Integration deleted', { id: req.params.id, tenantId: req.orgId });
  ...
  try {
    ...
    const scheduleResult = await executeSchedulingScript(
      ...
      samplePayload,
      {
        ...
        tenantId: integration?.tenantId || req.orgId,
