...
router.get('/integration/:__KEEP_integrationName__/versions', async (req, res) => {
  try {
    ...
    const integrationVersions = integrations
      .filter(integration =>
        integration.tenantId === req.orgId &&
...
// Helper functions
async function getIntegrationVersions(tenantId, __KEEP_integrationName__) {
  try {
    const integrations = await data.listIntegrations(tenantId);
    return integrations
      .filter(integration => integration.name === __KEEP_integrationName__)
      .map(integration => ({
        version: integration.metadata?.version || '1.0.0',
        isPrerelease: integration.metadata?.isPrerelease || false
      }))
...
async function updateDefaultVersionForName(tenantId, __KEEP_integrationName__, version, excludeIntegrationId = null) {
  try {
    const integrations = await data.listIntegrations(tenantId);
    ...
    for (const update of updates) {
      await data.updateIntegration(tenantId, update.id, { isDefault: update.isDefault, metadata: update.metadata });
    }
