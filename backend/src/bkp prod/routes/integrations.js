...
router.post('/', async (req, res) => {
  try {
    ...
    // For HTTP integrations (default), targetUrl is required
    // For COMMUNICATION integrations, actions array is required
    const hasCommunicationAction = actions && Array.isArray(actions) && actions.some(a => a.kind === 'COMMUNICATION');
    if (!hasCommunicationAction && !targetUrl) {
      return res.status(400).json({
        success: false,
        error: 'Missing required field: targetUrl (required for HTTP integrations)'
      });
    }
      ...
      if (integration.responseTransformation && integration.responseTransformation.script) {
        try {
          transformedResponse = await applyResponseTransform(
            ...
            { tenantId: integration.tenantId, orgId: req.orgId }
          );
        } catch (error) {
          return res.status(400).json({
            success: false,
...
const handleInboundRuntime = async (req, res) => {
  ...
  const tenantId = Number(orgId || req.orgId);
  ...
  try {
    ...
    if (!config) {
      ...
      return res.status(404).json({
        ...
        message: `No active integration found for type '${type}' and tenantId ${orgId}`
      });
    }
    ...
    if (config.rateLimits && config.rateLimits.enabled) {
      ...
      try {
        const rateResult = await checkRateLimit(config._id.toString(), tenantId, config.rateLimits);
...
async function logIntegration(config, status, details) {
  try {
    ...
    await data.recordLog(config.tenantId, {
