 ...
 * @query orgId (required), type, tenantId, category, isActive, search, limit
 */
router.get('/', asyncHandler(async (req, res) => {
  ...
  const filters = {
    type: req.query.type,
    tenantId: req.query.tenantId !== undefined ? parseInt(req.query.tenantId, 10) : undefined,
 ...
 * @query orgId (required)
 * @body type, source, target, description, category, tenantId, isActive
 */
 ...
 * @query orgId (required), mode (optional: "replace"), type, tenantId
 * @body Array of lookup objects
 */
router.post('/bulk', asyncHandler(async (req, res) => {
  const { orgId, mode, type, tenantId } = req.query;
  ...
  const options = {
    mode,
    type,
    tenantId: tenantId !== undefined ? parseInt(tenantId, 10) : undefined
  };
 ...
 * @query orgId (required), tenantId (optional)
 * @body { type, sourceId }
 */
router.post('/resolve', asyncHandler(async (req, res) => {
  const { orgId, tenantId } = req.query;
 ...
 */
router.post('/resolve-bulk', asyncHandler(async (req, res) => {
  const { orgId, tenantId } = req.query;
  ...
  const entRid = tenantId ? parseInt(tenantId, 10) : null;

  for (const sourceId of sourceIds) {
    const targetId = await resolveLookup(sourceId, type, parentRid, entRid);
    results.push({
      sourceId,
      targetId,
      found: targetId !== null
 ...
 * @query orgId (required), tenantId (optional), type (required)
 * @body { lookupConfigs: [...], samplePayload: {...} }
 */
router.post('/test', asyncHandler(async (req, res) => {
  const { orgId, tenantId, type } = req.query;
 ...
 * @query orgId (required), type (required), tenantId (optional), mode (optional: "replace")
 * @body multipart/form-data with 'file' field
 */
router.post('/import', upload.single('file'), asyncHandler(async (req, res) => {
  const { orgId, type, tenantId, mode } = req.query;
  ...
  try {
    ...
    const importResult = await bulkCreateLookups(
      parseInt(orgId, 10),
      parseResult.lookups,
      options
    );
