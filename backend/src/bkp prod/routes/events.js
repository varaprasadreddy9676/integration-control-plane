...
router.post('/test-notification-queue', asyncHandler(async (req, res) => {
  if (!db.isConfigured()) {
    return res.status(400).json({
  ...
  const resolvedParentRid = Number.isFinite(Number(bodyEntityParentRid))
    ? Number(bodyEntityParentRid)
    : req.orgId;
  const resolvedEntityRid = Number.isFinite(Number(bodyEntityRid))
    ? Number(bodyEntityRid)
    : resolvedParentRid;

  if (!resolvedEntityRid || !resolvedParentRid) {
    return res.status(400).json({
  ...
  const filteredEventTypes = Array.isArray(eventTypes) && eventTypes.length > 0
  ...
  const formatHms24 = (date) => `${pad2(date.getHours())}:${pad2(date.getMinutes())}:${pad2(date.getSeconds())}`;
  ...
  const applyOverrides = (payload) => {
    ...
    next.entityRID = resolvedEntityRid;
    next.entityParentID = resolvedParentRid;
    next.enterpriseEntityRID = resolvedParentRid;
  ...
  const rows = selectedEventTypes.map(eventType => {
    ...
    return {
      ...
      tenantId: resolvedEntityRid || null,
      orgId: resolvedParentRid || null,
  ...
  const sql = `INSERT INTO notification_queue (topic, transaction_type, message, entity_rid, entity_parent_rid, \`STATUS\`, created_at, delivered_at, last_checked_at, retry_count, error_message)
    VALUES (:topic, :transactionType, :message, :tenantId, :orgId, :status, :createdAt, :deliveredAt, :lastCheckedAt, :retryCount, :errorMessage)`;

  let inserted = 0;
  for (const row of rows) {
    // Validate that no parameters are undefined
    const hasUndefined = Object.entries(row).find(([key, value]) => value === undefined);
    if (hasUndefined) {
  ...
  log('info', 'Inserted test events into notification_queue', {
    tenantId: resolvedEntityRid,
    orgId: resolvedParentRid,
    inserted
  });
...
async function processImport(events, orgId, options = {}) {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    try {
      ...
      const payloadId = eventData.payload.id ||
      ...
      const eventKey = `${eventData.eventType}-${payloadId}-${eventData.tenantId}`;
