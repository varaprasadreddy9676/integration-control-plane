...
async function createIndexes() {
  try {
    ...
    await db.collection('integration_configs').createIndexes([
      { key: { tenantId: 1, type: 1 }, name: 'entity_event_idx' },
      { key: { tenantId: 1, isActive: 1 }, name: 'entity_active_idx' },
      { key: { tenantId: 1, direction: 1 }, name: 'tenant_direction_idx' },
    ...
    await db.collection('delivery_attempts').createIndexes([
      ...
      { key: { tenantId: 1, attemptedAt: -1 }, name: 'entity_attempted_idx' },
    ...
    await db.collection('processed_events').createIndexes([
      ...
      { key: { tenantId: 1, eventType: 1 }, name: 'entity_event_idx' },
    ...
    await db.collection('integration_templates').createIndexes([
      { key: { tenantId: 1, category: 1 }, name: 'entity_category_idx' },
      { key: { tenantId: 1, updatedAt: -1 }, name: 'entity_updated_idx' },
    ...
    await db.collection('alert_center_logs').createIndexes([
      { key: { tenantId: 1, createdAt: -1 }, name: 'alert_entity_created_idx' },
    ...
    await db.collection('lookups').createIndexes([
      ...
      // Reverse lookup index (with tenantId for hierarchy support)
      {
        key: {
          orgId: 1,
          tenantId: 1,
    ...
    await db.collection('scheduled_job_logs').createIndexes([
      { key: { tenantId: 1, startedAt: -1 }, name: 'entity_started_idx' },
