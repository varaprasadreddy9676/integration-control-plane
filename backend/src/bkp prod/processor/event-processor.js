...
async function processEvent(evt, pollCount = 0) {
  ...
  try {
    ...
    const integrations = await data.listIntegrationsForDelivery(evt.tenantId, evt.event_type);
    ...
    if (cancellationEvents.includes(evt.event_type)) {
      ...
      if (cancellationInfo) {
        ...
        const cancelledCount = await data.cancelScheduledIntegrationsByMatch(evt.tenantId, cancellationInfo);
    ...
    for (const integration of scheduledIntegrations) {
      try {
        ...
        if (transformed === null) {
          ...
          // eslint-disable-next-line no-await-in-loop
          const logId = await data.recordLog(evt.tenantId, {
