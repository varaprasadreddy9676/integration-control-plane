...
async function resolveMultiActionDelayMs(tenantId) {
  ...
  try {
    const uiConfig = await data.getUiConfigForEntity(tenantId);
...
async function deliverSingleAction(integration, action, evt, pollCount = 0, actionIndex = 0, correlationId = null, executionLogger = null, options = {}) {
  ...
  const maybeCreateActionDLQ = async ({ logId, errorMessage, errorCode, responseStatus, responseBody }) => {
    ...
    try {
      await dlqData.createDLQEntry({
        ...
        orgId: evt.orgId || integration.tenantId,
  ...
  if (!isCommunicationAction) {
    ...
    if (!urlCheck.valid) {
      ...
      const logId = await data.recordLog(integration.tenantId, {
        id: existingLogId,
  ...
  if (integration.rateLimits && integration.rateLimits.enabled) {
    ...
    try {
      const rateResult = await checkRateLimit(integration.id, evt.orgId || integration.tenantId, integration.rateLimits);
...
async function deliverMultiActionIntegration(integration, evt, pollCount = 0, correlationId = null, executionLogger = null) {
  ...
  const multiActionDelayMs = await resolveMultiActionDelayMs(integration.tenantId);
