...
async function processRetries(pollCount = 0, dbTimeout = 30000) {
  ...
  try {
    const retryLogs = await withTimeout(
      data.getFailedLogsForRetry(retryBatchSize),
      dbTimeout,
    ...
    for (const logEntry of retryLogs) {
      ...
      try {
        ...
        if (!integration || !integration.isActive) {
          await withTimeout(
            data.markLogAsAbandoned(logEntry.id),
            dbTimeout,
            'markLogAsAbandoned'
          ).catch(err => {
      ...
      let delaySeconds = 10 * Math.pow(2, logEntry.attemptCount - 1);
      ...
      if (new Date() < nextAttemptTime) {
        log('debug', 'Retry not ready yet', {
          ...
          remainingSeconds: Math.floor((nextAttemptTime - new Date()) / 1000)
        });
      ...
      if (action) {
        ...
        await deliverSingleAction(
          integration,
          action,
          retryEvent,
          pollCount,
          logEntry.actionIndex,
          traceId,
          null,
          { existingLogId: logEntry.id, triggerType: logEntry.triggerType || 'EVENT' }
        );
      } else {
        // eslint-disable-next-line no-await-in-loop
...
async function replayEvent(logId, tenantId, options = {}) {
  try {
    log('info', 'Starting event replay', { logId, tenantId, options });

    // Get original log entry
    const originalLog = await data.getLogById(tenantId, logId);
    if (!originalLog) {
    ...
    const replayEvent = {
      ...
      isReplay: true,
      originalLogId: logId,
      orgId: originalLog.tenantId || tenantId,
      tenantId: originalLog.tenantId || tenantId
    };
    ...
    const forceDelivery = Boolean(options.force || options.forceDelivery || options.ignoreCircuit || options.bypassCircuit);
    ...
    const action = actionIndex !== null && Array.isArray(integration.actions)
      ? integration.actions[actionIndex]
      : null;
    ...
    if (actionIndex !== null) {
      ...
      result = await deliverSingleAction(
        integration,
        action,
        replayEvent,
        0,
        ...
        replayCorrelationId,
        null,
        { existingLogId: logId, triggerType: 'REPLAY', retryReason: manualReason }
      );
    } else {
      result = await deliverToIntegration(integration, replayEvent, true, 0, logId, replayCorrelationId, false, { forceDelivery, retryReason: manualReason });
    }
    ...
    try {
      ...
      await data.recordLog(options.tenantId || 1, {
