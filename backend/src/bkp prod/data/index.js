...
async function getAllowedParentRids() {
  ...
  const parents = await dbClient.collection('integration_configs').distinct('tenantId', { isActive: true });
  allowedParentsCache.values = parents;
...
async function initDataLayer() {
  ...
  // Connect to MySQL for notification_queue reading only
  // Note: MySQL is optional - app can run without it (delivery worker will be disabled)
  if (useMysql()) {
    ...
    if (mysqlAvailable) {
      ...
      log('warn', 'MySQL ping failed - delivery worker will be disabled until connection is restored');
      log('warn', 'API and UI will continue to work normally');
      // Start reconnection attempts
      startMysqlReconnection(30000);
    }
  } else {
    log('warn', 'MySQL not configured - delivery worker will be disabled');
    mysqlAvailable = false;
  }
}
...
function mapIntegrationFromMongo(doc) {
  return {
    ...
    entityName: doc.entityName || doc.tenantId,
    scope: doc.scope,
...
async function listIntegrationsForDelivery(tenantId, eventType = null) {
  ...
  const parentRid = await getParentRidForEntity(tenantId);
  ...
  const direct = await listIntegrations(tenantId);
  ...
  if (parentRid && parentRid !== tenantId) {
    ...
    const inheritableParentHooks = parentHooks.filter((wh) => {
      ...
      if (wh.excludedEntityRids && wh.excludedEntityRids.length > 0) {
        if (wh.excludedEntityRids.includes(tenantId)) {
...
async function getParentRidForEntity(tenantId) {
  if (useMongo()) {
    try {
      const dbClient = await mongodb.getDbSafe();
      ...
      logError(err, { scope: 'getParentRidForEntity:mongo', tenantId });
    }
    return tenantId;
  }
  ...
  if (useMongo()) {
    try {
      ...
      const integration = {
        ...
        tenantId: payload.tenantId || orgId,
      ...
      logError(err, { scope: 'addIntegration', orgId });
    }
  }
...
async function updateIntegration(orgId, id, patch) {
  if (useMongo()) {
    try {
      ...
      return updated && updated.tenantId === orgId ? updated : undefined;
    } catch (err) {
 ...
 */
async function getIntegrationByTypeAndDirection(tenantId, type, direction) {
  if (useMongo()) {
    try {
      const db = await mongodb.getDbSafe();
      ...
      logError(err, { scope: 'getIntegrationByTypeAndDirection', tenantId, type, direction });
    }
  }
...
async function listEventTypes() {
  ...
  if (useMongo()) {
    try {
      ...
      // Project both type and eventType for backward compatibility
      const eventTypes = await collection.find({}, { projection: { type: 1, eventType: 1, _id: 0 } })
        .sort({ type: 1, eventType: 1 })
        .toArray();
      // Return type if exists, otherwise fallback to eventType for unmigrated docs
      return eventTypes.map(doc => doc.type || doc.eventType).filter(Boolean);
    } catch (err) {
...
async function getFailureReportSchedulerStatus(orgId) {
  if (useMongo()) {
    try {
      const uiConfig = await getUiConfigForEntity(orgId);
      ...
      const lastLog = orgId
        ? await db.collection('alert_center_logs')
          .find({ tenantId: orgId, type: 'DELIVERY_FAILURE_REPORT' })
          .sort({ createdAt: -1 })
          .limit(1)
          .toArray()
        : [];
    ...
    try {
      ...
      try {
        const attempts = await db.collection('delivery_attempts')
          .find({ deliveryLogId: id, tenantId: orgId })
...
async function recordLog(orgId, logPayload) {
  if (useMongo()) {
    try {
      ...
      const __KEEP___KEEP_integrationConfig__Id__Obj = mongodb.toObjectId(logPayload.__KEEP___KEEP_integrationConfig__Id__);
      if (existingLogId) {
        const attemptCount = logPayload.attemptCount || 1;

        // Fetch integration config to populate missing fields
        let integrationName = logPayload.__KEEP_integrationName__;
        ...
        const updateDoc = {
          ...
          webhookConfigId: __KEEP___KEEP_integrationConfig__Id__Obj || logPayload.__KEEP___KEEP_integrationConfig__Id__ || null,
          ...
          integrationConfigId: __KEEP___KEEP_integrationConfig__Id__Obj || logPayload.__KEEP___KEEP_integrationConfig__Id__ || null,
          __KEEP___KEEP_integrationConfig__Id__: __KEEP___KEEP_integrationConfig__Id__Obj || logPayload.__KEEP___KEEP_integrationConfig__Id__ || null,
          searchableText,
        ...
        const updateQuery = queryField === 'traceId'
          ? { traceId: existingLogId, orgId }
          : { _id: existingLogId, orgId };
  ...
  if (useMysql()) {
    try {
      ...
      return rows.map((row) => {
        ...
        return {
          id: row.id,
          tenantId: row.entity_rid,          // Renamed from entity_rid
...
async function listCustomTemplates(orgId) {
  if (useMongo()) {
    try {
      ...
      const templates = await collection.find({ tenantId: orgId })
        .sort({ updatedAt: -1 })
        .toArray();
  ...
  if (useMongo()) {
    try {
      ...
      await collection.updateOne(
        { _id: new ObjectId(templateId), tenantId: orgId },
        { $set: updateDoc }
      );

      return getCustomTemplate(orgId, templateId);
    } catch (err) {
...
async function createScheduledIntegration(data) {
  ...
  try {
    ...
    const scheduledIntegration = {
      __KEEP___KEEP_integrationConfig__Id__: mongodb.toObjectId(data.__KEEP___KEEP_integrationConfig__Id__),
      __KEEP_integrationName__: data.__KEEP_integrationName__,
      tenantId: data.tenantId,
      orgId: data.orgId || data.tenantId,
  ...
  if (useMongo()) {
    try {
      ...
      if (filters.integrationConfigId || filters.__KEEP___KEEP_integrationConfig__Id__) {
        query.__KEEP___KEEP_integrationConfig__Id__ = mongodb.toObjectId(filters.integrationConfigId || filters.__KEEP___KEEP_integrationConfig__Id__);
      }
      if (filters.eventType) {
        query.eventType = filters.eventType;
      }
 ...
 * @param {number} tenantId - Entity RID
 ...
 */
async function cancelScheduledIntegrationsByMatch(tenantId, matchCriteria) {
  if (!useMongo()) {
    return 0;
  }
...
async function deleteScheduledIntegration(orgId, id) {
  ...
  try {
    ...
    log('info', 'Scheduled integration cancelled', { id, orgId });
    return result.modifiedCount > 0;
  } catch (err) {
    logError(err, { scope: 'deleteScheduledIntegration', id, orgId });
    return false;
  }
}
 ...
 */
async function saveProcessedEvent(eventKey, mysqlEventId, eventType, tenantId, stableEventId = null) {
  if (!useMongo()) {
...
async function addLookup(orgId, payload) {
  ...
  try {
    ...
    logError(err, { scope: 'addLookup', orgId });
    throw err;
  }
}
...
async function updateLookup(orgId, id, patch) {
  ...
  try {
    ...
    return updated && updated.orgId === orgId ? updated : null;
  } catch (err) {
    // Handle duplicate key error
    if (err.code === 11000) {
      throw new Error('Duplicate lookup: A mapping with the same key already exists');
...
async function bulkCreateLookups(orgId, lookups, options = {}) {
  ...
  try {
    ...
    if (mode === 'replace' && type) {
      ...
      log('info', 'Deactivated existing lookups for replace mode', { orgId, type, tenantId });
    }
    ...
    for (const lookupData of lookups) {
      try {
        ...
        const existingQuery = {
          orgId: lookupData.orgId || orgId,
          tenantId: lookupData.tenantId || null,
 ...
 */
async function resolveLookup(sourceId, type, orgId, tenantId) {
  if (!useMongo()) {
    return null;
  }
 ...
 */
async function reverseLookup(targetId, type, orgId, tenantId) {
  if (!useMongo()) {
    return null;
  }
...
function extractSourceMetadata(event, sourceType) {
  ...
  if (sourceType === 'mysql') {
    ...
    if (event.entity_rid) metadata.tenantId = event.entity_rid;
...
function getSourceIdentifier(event, sourceType) {
  const type = sourceType || config.eventSource?.type || 'mysql';
 ...
 */
async function getEventTypeSamplePayload(eventType) {
  try {
    const db = await mongodb.getDbSafe();
    // Query by type field (new) with fallback to eventType (legacy)
    const eventTypeDoc = await db.collection('event_types').findOne(
      {
        $or: [
      ...
      { projection: { samplePayload: 1 } }
    );

    if (eventTypeDoc && eventTypeDoc.samplePayload) {
      log('debug', 'Event type sample payload retrieved', {
        eventType,
        hasSamplePayload: true
      });
      return eventTypeDoc.samplePayload;
    }
...
async function listTenantIds() {
  ...
  const sources = await Promise.all([
    dbClient.collection('integration_configs').distinct('tenantId'),
