...
async function getAllTemplates(entityRid) {
  try {
    return await data.listCustomTemplates(entityRid);
  } catch (err) {
...
async function getTemplateById(entityRid, templateId) {
  try {
    return await data.getCustomTemplate(entityRid, templateId);
  } catch (err) {
...
async function getTemplatesByCategory(entityRid, category) {
  try {
    const templates = await data.listCustomTemplates(entityRid);
...
async function getTemplateCategories(entityRid) {
  try {
    const templates = await data.listCustomTemplates(entityRid);
...
async function createIntegrationFromTemplate(entityRid, templateId, overrides = {}) {
  const template = await getTemplateById(entityRid, templateId);
  if (!template) {
...
async function validateTemplate(entityRid, templateId, overrides = {}) {
  const errors = [];
  const warnings = [];
  const template = await getTemplateById(entityRid, templateId);
  ...
  try {
    __KEEP_integrationConfig__ = await createIntegrationFromTemplate(entityRid, templateId, overrides);
  } catch (err) {
