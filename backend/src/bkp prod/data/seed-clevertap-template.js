...
/**
 * Seed CleverTap template for a specific entity
 */
async function seedCleverTapTemplate(entityParentRid) {
  try {
    await mongodb.connect();
    const mongoDb = await mongodb.getDbSafe();
    const collection = mongoDb.collection('integration_templates');

    log('info', 'Seeding CleverTap template', { entityParentRid });
...
/**
 * Seed template for all existing entities
 */
async function seedCleverTapTemplateForAll() {
  try {
    ...
    // Get unique entityRids from existing integrations
    const entityRids = await integrationsCollection.distinct('entityRid');

    log('info', 'Found entities to seed templates for', {
      count: entityRids.length,
      entityRids
    });

    const results = [];
    for (const entityRid of entityRids) {
      try {
        const result = await seedCleverTapTemplate(entityRid);
        results.push({
          entityRid,
          success: true,
...
if (require.main === module) {
  ...
  const entityRid = args[0] ? parseInt(args[0]) : null;

  if (entityRid) {
    // Seed for specific entity
    seedCleverTapTemplate(entityRid)
      .then((result) => {
        console.log('✅ CleverTap template seeded successfully:');
        console.log(`   Template ID: ${result.id}`);
        console.log(`   Entity RID: ${entityRid}`);
    ...
    seedCleverTapTemplateForAll()
      .then((results) => {
        ...
        results.forEach(result => {
          if (result.success) {
            console.log(`   ✓ Entity ${result.entityRid}: ${result.isNew ? 'Created' : 'Updated'} (${result.templateId})`);
          } else {
            console.log(`   ✗ Entity ${result.entityRid}: ${result.error}`);
          }
        });
