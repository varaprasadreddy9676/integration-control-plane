...
function getTenant(entityParentRid) {
  return state.tenants.find((tenant) => tenant.entityParentRid === entityParentRid);
}
...
function getPermittedEntityRids(entityParentRid) {
  const tenant = getTenant(entityParentRid);
  if (!tenant) return [];
  const childIds = tenant.childEntities?.map((c) => c.rid) ?? [];
  return [tenant.entityParentRid, ...childIds];
}
...
function isIntegrationInTenant(integration, entityParentRid) {
  const allowed = new Set(getPermittedEntityRids(entityParentRid));
  return allowed.has(integration.entityRid);
}

function listIntegrations(entityParentRid) {
  const allowed = new Set(getPermittedEntityRids(entityParentRid));
  return state.integrations.filter((wh) => allowed.has(wh.entityRid));
}

async function addIntegration(entityParentRid, payload) {
  const tenant = getTenant(entityParentRid);
  const integration = {
    id: payload.id || `wh_${randomUUID()}`,
    name: payload.name,
    eventType: payload.eventType,
    entityRid: payload.entityRid || entityParentRid,
...
async function updateIntegration(entityParentRid, id, patch) {
  const idx = state.integrations.findIndex((wh) => wh.id === id && isIntegrationInTenant(wh, entityParentRid));
  ...
  const updated = {
    ...
    ...patch,
    id: existing.id,
    entityRid: patch.entityRid || existing.entityRid,
...
async function deleteIntegration(entityParentRid, id) {
  const before = state.integrations.length;
  state.integrations = state.integrations.filter((wh) => !(wh.id === id && isIntegrationInTenant(wh, entityParentRid)));
...
function getLogById(entityParentRid, id) {
  return listLogs(entityParentRid).find((log) => log.id === id);
}

async function recordLog(entityParentRid, log) {
...
function listApiKeys(entityParentRid) {
  return state.apiKeys
    .filter((key) => key.entityRid === entityParentRid)
    .map((key) => {
...
async function addApiKey(entityParentRid, description) {
  const keyValue = `mdcs_${randomUUID().replace(/-/g, '').slice(0, 16)}`;
  const newKey = {
    id: `ak_${randomUUID()}`,
    entityRid: entityParentRid,
    description,
    key: keyValue,
    isActive: true,
    createdAt: new Date().toISOString()
...
async function toggleApiKey(entityParentRid, id) {
  const idx = state.apiKeys.findIndex((key) => key.id === id && key.entityRid === entityParentRid);
  if (idx === -1) return undefined;
...
function getPendingEvents(limit = 5) {
  return queue.take(limit).map((evt) => ({
    id: evt.id,
    entity_rid: evt.tenantId,
    event_type: evt.event_type,
    payload: evt.payload
  }));
}

function getDashboardSummary(entityParentRid) {
  const logs = listLogs(entityParentRid);
  ...
  const integrationHealth = listIntegrations(entityParentRid).slice(0, 5).map((wh, idx) => ({
    id: wh.id,
    name: wh.name,
