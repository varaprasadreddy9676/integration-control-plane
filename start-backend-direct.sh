...
echo "Stopping existing backend server (if any)..."
if [ -f "${PID_FILE}" ]; then
  OLD_PID=$(cat "${PID_FILE}")
  if ps -p "${OLD_PID}" > /dev/null 2>&1; then
    echo "Found running server (PID ${OLD_PID}), stopping..."
...
echo "Starting backend (no supervisor)..."
cd "${ROOT_DIR}/backend"
nohup node src/index.js > "${ROOT_DIR}/backend.nohup.out" 2>&1 &
echo $! > "${PID_FILE}"
